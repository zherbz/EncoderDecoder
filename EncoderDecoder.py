...
def build_model(data, config, output_length, X_test, input_length, index, repeats):
	...
	model.add(LSTM(n_nodes1, return_sequences=True))
	model.add(ReLU())
	model.add(TimeDistributed(Dense(n_nodes2)))
	model.add(ReLU())
...
def get_stats(y_pred, y_test):
	...
	print('Nash-Sutcliffe Efficiency:', round(r2_score(y_test, y_pred), 3))
...
def get_stats_df(y_pred, y_test):
	...
	index = ['Mean Absolute Error (%)', 'Root Mean Squared Error (%)', 'Median Absolute Error (%)', 'Nash-Sutcliffe Efficiency', 'Explained Variance']
	return stats, index
...
data = pickle.load(open(r'C:\Users\Username\Documents\Project Data\project_data.pkl', 'rb'))
sv = data['Upper Stillwater']
swe = data[data.columns[1:]]

# Declare model input parameters
# Note: setting repeats to 1 will return an error becuase the regression metrics need a sample size greater than 1 to average across
