...
# Define function to calculate the absolute percent error for each forecast step
def mape(y_test, y_pred): 
    y_test, y_pred = np.array(y_test), np.array(y_pred)
    return np.mean(np.abs((y_test - y_pred) / y_test))*100, np.abs((y_test - y_pred) / y_test)*100

# Define function to create sliding window input by index
def sliding_window_input(df, input_length, idx):
	window = df.values[idx:input_length+idx,:]
	return window

# Define function to create sliding window output by index
...
# Define function to configure model hyperparameters
def model_configs():
	epochs = [100]
	batch_size = [32]
	n_nodes1 = [64]
...
# Define function to build Encoder-Decoder model
def build_model(data, config, output_length, X_test, input_length, index, repeats):
	...
	# define model
	model = Sequential()
	# Encoder: Stacked Convolutional Nueral Network responsible for extracting important features from input sequence
	model.add(Conv1D(filter1, kernel_size, input_shape=(n_timesteps,n_features)))
	model.add(layers.Activation('relu'))
	model.add(Conv1D(filter2, kernel_size))
	model.add(layers.Activation('relu'))
	model.add(Conv1D(filter2, kernel_size))
	model.add(layers.Activation('relu'))
	...
	# Decoder: Stacked LSTM responsible for interpreting the extracted features from the CNN
	model.add(LSTM(n_nodes1, return_sequences=True))
	model.add(layers.Activation('relu'))
	model.add(LSTM(n_nodes1, return_sequences=True))
	model.add(layers.Activation('relu'))
	model.add(LSTM(n_nodes1, return_sequences=True))
	...
	model.add(LSTM(n_nodes1, return_sequences=True))
	model.add(layers.Activation('relu'))
	model.add(TimeDistributed(Dense(n_nodes2, activation='relu')))
...
# Define function to produce regression metrics against a hold-out test set
...
# Define function to produce a dataframe of regression metrics against a hold-out test set
def get_stats_df(y_pred, y_test):
	...
	stats = [[Meanae], [RMSE], [Medae], [R2], [Expvar]]
	index = ['Mean Absolute Error (%)', 'Root Mean Squared Error (%)', 'Median Absolute Error (%)', 'R-squared', 'Explained Variance']
	return stats, index

# Define function to calculate the absolute percent error
def ape(true, test):
	value = (abs(true - test)/true)*100
	return np.array(value)

# Define function to repeatedly build and trian the Encoder-Decoder model to then plot learning curves and forecasts
def repeat_runs(repeats, data, cfg, output_length, X_test, input_length, y_test):
	...
	upper = avg + 1.96*std/(repeats**(0.5))
	lower = avg - 1.96*std/(repeats**(0.5))
	...
	cellText, rows = get_stats_df(y_test.values, avg.reshape(-1,))
	...
	return df, history
# %%
# Load pkl file of project data containing storage volume (ac-ft) and snow water equivalent (in) time series data
data = pickle.load(open(r'C:\Users\Zachary\Documents\Project Data\project_data.pkl', 'rb'))
sv = data['Upper Stillwater']
swe = data[data.columns[1:]]

# Declare model input parameters
# Note: setting repeats to 1 will return an error becuase the regression metrics need a sample size greater than 1
input_length, output_length, repeats, year = 20, 15, 2, 2019
